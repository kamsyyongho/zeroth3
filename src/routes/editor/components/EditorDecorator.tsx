...
import { DEFAULT_OFFSET, ENTITY_TYPE, LOCAL_STORAGE_KEYS, MUTABILITY_TYPE, WordAlignmentEntityData, WordAlignment } from '../../../types';
...
export const EditorDecorator = (props: EntityContentProps) => {
    const {
        // contentState,
        wordAlignment } = props;
    ...
    const renderPopper = (curretRef: HTMLButtonElement | null, isPlayingBlock: boolean = true, isVisible: boolean) => {
        if (!curretRef) {
            return null;
        }
        return (<Popper
    ...
    const content = (
        ...
        >
            {({ isVisible }) => {
                return (<span ref={segmentRef} style={style}>
          {props.children}
